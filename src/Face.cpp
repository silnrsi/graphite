...
{
    size_t sz = 0;
    _p = static_cast<const uint8_t *>((*_f->m_ops.get_table)(_f->m_appFaceHandle, n, &sz));
    _sz = uint32_t(sz);
...
Error Face::Table::decompress()
{
    ...
    case LZ4:
    {
        ...
        uncompressed_table = gralloc<uint8_t>(uncompressed_size);
