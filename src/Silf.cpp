...
bool Silf::readGraphite(const uint8_t * const silf_start, size_t lSilf, Face& face, uint32_t version)
{
    const uint8_t * p = silf_start,
...
template<typename T> inline uint32_t Silf::readClassOffsets(const uint8_t *&p, size_t data_len, Error &e)
{
...
size_t Silf::readClassMap(const uint8_t *p, size_t data_len, uint32_t version, Error &e)
{
