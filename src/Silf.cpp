...
bool Silf::readGraphite(void *pSilf, size_t lSilf, int numGlyphs, uint32 version)
{
    ...
    XmlTraceLog::get().addAttribute(AttrNumPseudo, numPseudo);
    ...
    for (int i = 0; i < m_numPasses; i++)
    {
        ...
        if (!m_passes[i].readPass((char *)pSilf + swap32(pPasses[i]), swap32(pPasses[i + 1]) - swap32(pPasses[i]), numGlyphs + numPseudo))
        {
