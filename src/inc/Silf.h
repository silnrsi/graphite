...
public:
    ...
    bool readGraphite(const uint8_t * const pSilf, size_t lSilf, Face &face, uint32_t version);
    ...
    uint16_t numClasses() const { return m_nClass; }
...
private:
    size_t readClassMap(const uint8_t *p, size_t data_len, uint32_t version, Error &e);
    template<typename T> inline uint32_t readClassOffsets(const uint8_t *&p, size_t data_len, Error &e);
